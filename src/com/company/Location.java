...
public class Location {
    ...
    public int getTaxiCost() {return taxiCost;}            // A public function to get the address of the location
     ...
     public void locationInput(String[] uberLocation, int currentWalletBalance) {
            ...
            try {                   //Try prevents the input to be of an invalid value
                while (running) {
                    ...
                    for (int i = 0; i < uberLocation.length; i++ ) {
                         if ((menuChoice.equalsIgnoreCase(uberLocation[i])) && (currentWalletBalance>taxiCost)) {
                             flag += 1;
    ...
    public boolean canAffordTaxi(int currentWalletBalance) {
        boolean canAfford = true;
        if (currentWalletBalance < taxiCost) {
            canAfford = false;
        }
        return canAfford;
    }
